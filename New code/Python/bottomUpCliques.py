...
def bottom_up_cliques(D, iter_, clean_up = True):
    ...
    if(clean_up):
        for singleton in singletons: #for each singleton left
            ...
            if clique_id < 0: # there's a change the best_fit is a singleton, though it's very unlikely
                cliques.append[set([singleton, best_fit])]
                clique_thresh = len(D[singleton, best_fit])
                reference_matrix[clique_thresh,singleton] = clique_id
                reference_matrix[clique_thresh,best_fit] = clique_id
            else:
        ...
        if(cliques_[i]):
            for j in range(len(cliques_[i])):
                ...
                with open('cliques/'+clique_id+".clique", 'w') as f:
                    for mem in members:
                        f.write(names[mem]+'\n')
                        new_index.append([ids[mem], names[mem], cluster_id, clique_id])
...
def main(args):
    ...
    threshold = 5 #How big an atom has to be to be broken up (bigger than threshold)
    iter_ = 4 #At which distance to start joining singletons to nearest cliques
