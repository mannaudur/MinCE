    ...
    size_t iter_)
{
    ...
    std::cout << "Size of options: " << size_of_options << std::endl;
        ...
        for(int i = 0; i < chosen_vector.size(); i++) {
            if(chosen_vector[i]){ // For every member taking that sequence
                ...
                if(value -> size() == 1) { // If we have no more sequences associated with this vector form, except for this one
                    khiter_t h;
                    std::cout << "Removing " << chosen_vector_ind << " from connection to member " << i << std::endl;
                    ...
                    for(auto el : *value_connected) {
                        std::cout << el << " ";
                    }
                    std::cout << std::endl;
                    std::remove((*value_connected).begin(), (*value_connected).end(), chosen_vector_ind); // Remove connection from member to vector from (ONLY for singles)
    ...
    std::cout << "Updated size of options: " << value->size() << std::endl;
}
    ...
    std::vector<int>* goal_vector)
{
    ...
    for (int row_num = 0; row_num < uniques->size(); row_num++)
    {
        ...
        for(size_t i = 0; i < row_vector.first.size(); i++) {
            if(row_vector.first[i] && (*connections)[i][i] < N+1) { // For 1 in results
                std::cout << "Found unique focus member " << i << std::endl;
                ...
                std::remove((*value_connected).begin(), (*value_connected).end(), row_num); // Remove connection from member to vector from (ONLY for singles)
                break;
            }
        }
    }
    std::cout << "Unique matrix is: " << std::endl;
    ...
    while(goalsRemaining(connections, goal_vector, N)) {
        std::cout << "Connection matrix is: " << std::endl;
        ...
        std::cout << "Found focus member " << focus_member << std::endl;
