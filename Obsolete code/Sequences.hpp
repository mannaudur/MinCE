    ...
    size_t iter_)
{
    ...
    uint64_t vector_form = convertToNumber(chosen_vector); // Get vector id number for this form
    ...
    uint64_t chosen_sequence; // Pick the first one (doesn't matter which)
    for(int j = 0; j < min(iter_, value->size()); j++) {
        chosen_sequence = (*value)[j];
    ...
    std::vector<int>* goal_vector)
{
    ...
    while(goalsRemaining(connections, goal_vector, N)) {
        ...
        logSequenceToMembers(focus_row, chosen, fasta_mapping, connected_to, connections, uniques, 1);
    }
            ...
            if((*connections)[i][i] == (*connections)[i][j]) 
            {
                ...
                if (k != kh_end(connected_to))
                {
                    ...
                    for(int M = 0; M < min(final_adds.size(), C); M++) {
                        ...
                        logSequenceToMembers(focus_row, chosen, fasta_mapping, connected_to, connections, uniques, 3);
                    }
                }
                final_adds.clear();
